...
/**
 * 标准字体大小设置14像素（rem参照对象）
 * html根节点设置和浏览器一样高
 * 如果内容操作，滚动事件发生在html元素上
 * 背景颜色为白色
 */
 ...
 * 给html,body一样的高度，防止html高度小于body
 * 取消部分浏览器点击有阴影
 * 优化移动端滚动事件
 */
 ...
 * pre、code、legend、fieldset、blockquote……等其他标签不是很常用，所以本css都不会一一列举出来，为了简化如果项目中用到，可以自己单独写
 */
...
/**
 * 移除浏览器部分元素的默认边框
 * acronym、fieldset……等其他标签不是很常用，所以本css都不会一一列举出来，为了简化如果项目中用到，可以自己单独写
 */
